...
class JsonMockupObject : public RedfishObject {
 public:
  explicit JsonMockupObject(json_t *json_view)
      : json_view_(ABSL_DIE_IF_NULL(json_view)) {}
  RedfishVariant GetNode(const std::string &node_name) override {
    // TODO(dchanman): GetNode should check if the object retrieved is some
    // "@odata.id" object, in which case we should return the referenced object.
...
class JsonMockupMockup : public RedfishInterface {
 public:
  explicit JsonMockupMockup(absl::string_view raw_json) {
    ...
    if (!json_model_) {
      LOG(FATAL) << "Could not load JSON: " << err.text;
    }
  }
