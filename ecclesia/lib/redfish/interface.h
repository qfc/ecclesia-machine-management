...
class RedfishObject {
 public:
  ...
  virtual RedfishVariant GetNode(const std::string &node_name) = 0;
  ...
  template <typename T>
  absl::optional<T> GetNodeValue(absl::string_view node_name) {
    T val;
  ...
  absl::optional<typename PropertyDefinitionT::type> GetNodeValue() {
...
class RedfishInterface {
 public:
  ...
  virtual void UpdateEndpoint(absl::string_view endpoint) = 0;
...
class NullRedfish : public RedfishInterface {
  ...
  void UpdateEndpoint(absl::string_view endpoint) override {}
