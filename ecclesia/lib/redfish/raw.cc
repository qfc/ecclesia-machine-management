...
extern "C" {
#include "third_party/libredfish/include/redfishPayload.h"
#include "third_party/libredfish/include/redfishService.h"
}  // extern "C"
...
constexpr char kRedfishServiceVersionRoot[] = "v1";
...
class RawPayload {
 ...
 public:
  ...
  RawPayload(PayloadUniquePtr payload, ConstructorPasskey)
      : payload_(std::move(ABSL_DIE_IF_NULL(payload))) {}
...
class RawIntf : public RedfishInterface {
 public:
  explicit RawIntf(ServiceUniquePtr service) : service_(std::move(service)) {}
  ...
  RedfishVariant GetRoot() override {
    ...
    return RedfishVariant(absl::make_unique<RawVariantImpl>(
        RawPayload::NewShared(PayloadUniquePtr(getRedfishServiceRoot(
            service_.get(), kRedfishServiceVersionRoot)))));
  }
