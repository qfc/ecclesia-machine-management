...
class FirmwareSlotInfo {
 public:
  ...
  static absl::StatusOr<std::unique_ptr<FirmwareSlotInfo>> Parse(
    ...
    if (buf.size() != kFirmwareSlotInfoFormatSize) {
  ...
  Slot CurrentActiveFirmwareSlot() const {
    return static_cast<Slot>(firmware_slot_info_->afi & 0x7);
  }
  ...
  Slot NextActiveFirmwareSlot() const {
    return static_cast<Slot>(firmware_slot_info_->afi >> 4 & 0x7);
  }
  ...
  absl::flat_hash_map<Slot, std::string> FirmwareRevisions() const {
    ...
    for (int slot = kSlot1; slot < kSlotMax; ++slot) {
      int index = slot - 1;
      int len =
          strnlen(firmware_slot_info_->frs[index], kFirmwareRevisionMaxLen);
      revisions[static_cast<Slot>(slot)] =
          std::string(firmware_slot_info_->frs[index], len);
    }
    return revisions;
  }
 ...
 protected:
  explicit FirmwareSlotInfo(absl::string_view buf)
      : data_(buf),
        firmware_slot_info_(
            reinterpret_cast<const FirmwareSlotInfoFormat *>(data_.data())) {}

 private:
  const std::string data_;
  const FirmwareSlotInfoFormat *const firmware_slot_info_;
};
