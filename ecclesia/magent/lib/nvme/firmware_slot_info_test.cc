...
namespace ecclesia {
namespace {
constexpr unsigned char kFirmwareSlotInfoData[kFirmwareSlotInfoFormatSize] = {
...
TEST(FirmwareSlotInfoTest, ParseFirmwareSlotInfo) {
  auto ret = FirmwareSlotInfo::Parse(
      std::string(kFirmwareSlotInfoData,
                  kFirmwareSlotInfoData + kFirmwareSlotInfoFormatSize));
  ASSERT_TRUE(ret.ok());
