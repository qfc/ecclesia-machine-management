...
class DeviceSelfTestResult {
 public:
  ...
  static absl::StatusOr<DeviceSelfTestResult> Parse(absl::string_view buf) {
    if (buf.size() != kDeviceSelfTestResultFormatSize) {
  ...
  SelfTestCode DeviceSelfTestCode() const {
    return static_cast<SelfTestCode>(self_test_result_->status >> 4);
  }

  Result SelfTestResult() const {
    return static_cast<Result>(self_test_result_->status & 0xF);
  }
  ...
  absl::optional<uint8_t> FailedSegmentNumber() const {
    ...
    return self_test_result_->segment_number;
  }

  uint64_t PowerOnHours() const { return self_test_result_->power_on_hours; }

  // Namespace the Failing LBA occurred on if known.
  absl::optional<uint32_t> FailingNamespace() const {
    if (!self_test_result_->nsid_valid) return absl::nullopt;
    return LittleEndian::Load32(
        reinterpret_cast<const unsigned char *>(&self_test_result_->nsid));
  }

  // One of the failing LBAs if known.
  absl::optional<uint64_t> FailingLBA() const {
    if (!self_test_result_->flba_valid) return absl::nullopt;
    return LittleEndian::Load64(
        reinterpret_cast<const unsigned char *>(&self_test_result_->flba));
  }

  // Status code type if known.
  absl::optional<uint8_t> StatusCodeType() const {
    if (!self_test_result_->sct_valid) return absl::nullopt;
    return self_test_result_->status_code_type;
  }

  // Status code if known.
  absl::optional<uint8_t> StatusCode() const {
    if (!self_test_result_->sc_valid) return absl::nullopt;
    return self_test_result_->status_code;
  }

  uint16_t VendorSpecificField() const {
    return LittleEndian::Load16(reinterpret_cast<const unsigned char *>(
        &self_test_result_->vendor_specific));
  }
 ...
 protected:
  explicit DeviceSelfTestResult(absl::string_view buf)
      : data_(buf),
        self_test_result_(reinterpret_cast<const DeviceSelfTestResultFormat *>(
            data_.data())) {}

 private:
  std::string data_;
  const DeviceSelfTestResultFormat *self_test_result_;
};
...
class DeviceSelfTestLog {
 public:
  static absl::StatusOr<std::unique_ptr<DeviceSelfTestLog>> Parse(
    ...
    if (buf.size() != kDeviceSelfTestLogFormatSize) {
      return absl::InternalError("Unexpected error.");
    }
 ...
 protected:
  explicit DeviceSelfTestLog(absl::string_view buf)
      : data_(buf),
        log_(reinterpret_cast<const DeviceSelfTestLogFormat *>(data_.data())) {}

 private:
  const std::string data_;
  const DeviceSelfTestLogFormat *const log_;
};
}  // namespace ecclesia
