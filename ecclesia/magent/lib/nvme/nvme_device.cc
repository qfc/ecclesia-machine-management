...
class NvmeDevice : public NvmeDeviceInterface {
 public:
  ...
  absl::StatusOr<std::set<uint32_t>> EnumerateNamespacesAfter(
    ...
    // A buffer to hold the response structure which will be parsed.
    std::string buffer(kIdentifyListNamespaceSize, '\0');

    nvme_passthru_cmd cmd = {
        .opcode = kNvmeOpcodeAdminIdentify,
        .nsid = 0,
        .addr = reinterpret_cast<uint64_t>(buffer.c_str()),
        .data_len = kIdentifyListNamespaceSize,
  ...
  absl::StatusOr<std::string> InternalIdentifyNamespace(uint32_t namespace_id) {
    ...
    // A buffer to hold the response structure which will be parsed.
    std::string buffer(kIdentifyNamespaceSize, '\0');

    nvme_passthru_cmd cmd = {
        .opcode = kNvmeOpcodeAdminIdentify,
        .nsid = namespace_id,
        .addr = reinterpret_cast<uint64_t>(buffer.c_str()),
        .data_len = kIdentifyNamespaceSize,
  ...
  absl::StatusOr<std::vector<LBAFormat>> GetSupportedLbaFormats() override {
  ...
  absl::StatusOr<std::unique_ptr<SmartLogPageInterface>> SmartLog()
    ...
    auto ret = GetLogPage(LogPageIdentifier::kSmart, kSmartLogPageSize,
                          kAllNamespaces);
    if (!ret.ok()) {
      return absl::Status(
          ret.status().code(),
  ...
  absl::StatusOr<std::unique_ptr<SanitizeLogPageInterface>> SanitizeStatusLog()
    ...
    auto ret = GetLogPage(LogPageIdentifier::kSanitizeStatus,
                          kSanitizeLogPageSize, kAllNamespaces);
    if (!ret.ok()) {
      return absl::Status(
          ret.status().code(),
          absl::StrCat(ret.status().message(), ";",
  ...
  absl::StatusOr<std::vector<ErrorLogInfoEntry>> ErrorInformation()
      const override;
  ...
  absl::Status AttachNamespace(uint32_t namespace_id,
    ...
    ControllerListFormat controller_list{0};
    controller_list.num_identifiers = 1;
    controller_list.identifiers[0] = controller_id;

    nvme_passthru_cmd cmd = {
        .opcode = kNvmeOpcodeAdminAttachNs,
        .nsid = namespace_id,
        .addr = reinterpret_cast<uint64_t>(&controller_list),
        .data_len = sizeof(controller_list),
        .cdw10 = kAttachType,
        .cdw11 = 0,
        .cdw12 = 0,
        .cdw13 = 0,
  ...
  absl::StatusOr<uint32_t> CreateNamespace(uint64_t capacity_lba,
    ...
    NamespaceManagementFormat buffer{0};
    buffer.size = capacity_lba;
    buffer.capacity = capacity_lba;
    buffer.formatted_lba_size = lba_format_ix;

    nvme_passthru_cmd cmd = {
        .opcode = kNvmeOpcodeAdminNsMgt,
        .nsid = 0,
        .addr = reinterpret_cast<uint64_t>(&buffer),
        .data_len = kNamespaceManagementSize,
        .cdw10 = kMgtType,
        .cdw11 = 0,
        .cdw12 = 0,
        .cdw13 = 0,
  ...
  absl::Status SanitizeCryptoErase() override {
    ...
    nvme_passthru_cmd cmd = {
        ...
        .addr = 0,
        .data_len = 0,
        .cdw10 = static_cast<uint32_t>(kSanitizeCryptoErase),
        .cdw11 = 0,
    };
  ...
  absl::Status SanitizeBlockErase() override {
    ...
    nvme_passthru_cmd cmd = {
        ...
        .addr = 0,
        .data_len = 0,
        .cdw10 = static_cast<uint32_t>(kSanitizeBlockErase),
        .cdw11 = 0,
    };
  ...
  absl::Status SanitizeOverwrite(uint32_t overwrite_pattern) override {
    ...
    nvme_passthru_cmd cmd = {
        ...
        .data_len = 0,
        .cdw10 =
            static_cast<uint32_t>(kOverwritePassCountMask | kSanitizeOverwrite),
  ...
  absl::Status SanitizeExitFailureMode() override {
    ...
    nvme_passthru_cmd cmd = {
        ...
        .addr = 0,
        .data_len = 0,
        .cdw10 = static_cast<uint32_t>(kSanitizeExitFailureMode),
        .cdw11 = 0,
    };
...
NvmeDeviceInterface::EnumerateAllNamespaces() {
  ...
  while (true) {
    ...
    if (cmd_nsids.size() < kIdentifyListNamespacesCapacity) {
      break;
    }
...
absl::StatusOr<uint8_t> NvmeDeviceInterface::IndexOfFormatWithLbaSize(
  ...
  for (int8_t i = 0; i < supported_formats.size(); ++i) {
    const LBAFormat &format = supported_formats[i];
    uint64_t data_size_bytes = 1ULL << format.data_size;
    if (format.metadata_size == 0 && lba_size_bytes == data_size_bytes) {
      if (best_matching_format_index < 0) {
        best_matching_format_index = i;
      } else if (format.relative_performance <
...
absl::StatusOr<std::vector<ErrorLogInfoEntry>> NvmeDevice::ErrorInformation()
    const {
  auto ret = Identify();
  if (!ret.ok()) {
    return absl::Status(
  ...
  auto identify = std::move(ret.value());
  uint32_t len =
      identify->max_error_log_page_entries() * kErrorLogInfoEntrySize;
  static constexpr uint32_t kAllNamespaces = -1;

  auto ret2 =
      GetLogPage(LogPageIdentifier::kErrorInformation, len, kAllNamespaces);
  if (!ret2.ok()) {
    return absl::Status(ret2.status().code(), ret2.status().message());
  }
  std::string buffer = std::move(ret2.value());
  if (len != buffer.size()) return absl::InternalError("Unexpected error.");
  // Determine the number of valid entries
  const ErrorLogInfoEntry *entries =
      reinterpret_cast<ErrorLogInfoEntry *>(buffer.data());
  uint16_t index = 0;
  uint16_t num_entries_valid = 0;
  while (index < identify->max_error_log_page_entries()) {
    if (entries[index++].error_count == 0) {
      break;
    }
    ++num_entries_valid;
  }

  std::vector<ErrorLogInfoEntry> error_log_entries;
  error_log_entries.reserve(num_entries_valid);
  return std::vector<ErrorLogInfoEntry>(entries, entries + num_entries_valid);
}
...
NvmeDevice::FirmwareSlotInformation() const {
  ...
  auto ret = GetLogPage(LogPageIdentifier::kFirmwareSlotInformation,
                        kFirmwareSlotInfoFormatSize, kAllNamespaces);
  if (!ret.ok()) {
    return absl::Status(ret.status().code(), ret.status().message());
  }
...
NvmeDevice::GetDeviceSelfTestLog() const {
  ...
  auto ret = GetLogPage(LogPageIdentifier::kDeviceSelfTest,
                        kDeviceSelfTestLogFormatSize, kAllNamespaces);
  if (!ret.ok()) {
    return absl::Status(ret.status().code(), ret.status().message());
  }
  auto buffer = std::move(ret.value());
