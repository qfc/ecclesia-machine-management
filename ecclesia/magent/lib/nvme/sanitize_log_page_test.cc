...
constexpr unsigned char kBlockSanitizeLogData[kSanitizeLogPageSize] = {
...
// Estimated Time For Block Erase (No-Deallocate) :  0
// Estimated Time For Crypto Erase (No-Deallocate):  0
constexpr unsigned char kCryptoSanitizeLogData[kSanitizeLogPageSize] = {
  ...
  std::unique_ptr<SanitizeLogPageInterface> sanitize(
      SanitizeLogPage::Parse(
          std::string(kBlockSanitizeLogData,
                      kBlockSanitizeLogData + kSanitizeLogPageSize))
          .value());
  ...
  std::unique_ptr<SanitizeLogPageInterface> sanitize(
      SanitizeLogPage::Parse(
          std::string(kCryptoSanitizeLogData,
                      kCryptoSanitizeLogData + kSanitizeLogPageSize))
          .value());
  ASSERT_TRUE(sanitize.get());
...
TEST(SanitizeLogPageTest, SanitizeParseBufferFail) {
  ...
  EXPECT_THAT(status.ToString(),
              testing::HasSubstr("The sanitize log page size must be equal "
                                 "to kSanitizeLogPageSize."));
}
