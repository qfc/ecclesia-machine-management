...
// Cpu topology as observed on an Interlaken machine
...
class InterlakenVisitorTest : public ::testing::Test {
 protected:
  InterlakenVisitorTest() { reader_ = new MockEventReader(); }

  MockEventReader *reader_;
  absl::Notification last_event_logged_;
};

TEST_F(InterlakenVisitorTest, MemoryErrorCounts) {
  MachineCheck mces[] = {
      {
...

  {
    ...
    auto dimm_visitor = CreateInterlakenDimmErrorCountingVisitor(
...
TEST_F(InterlakenVisitorTest, CpuErrorCounts) {
  MachineCheck mces[] = {
      {
...

  {
    ...
    auto cpu_visitor = CreateInterlakenCpuErrorCountingVisitor(
        absl::UnixEpoch(), absl::make_unique<FakeCpuTopology>());
