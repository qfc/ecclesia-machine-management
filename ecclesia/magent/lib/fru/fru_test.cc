...
constexpr uint8 kFormatVersion = 1;

// This is a known-good FRU from cglr46.
static uint8_t prefab_frudata[] = {
    0x01, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0xfe, 0x01, 0x07, 0x00,
    0xb3, 0x1e, 0x63, 0xc7, 0x51, 0x75, 0x61, 0x6e, 0x74, 0x61, 0x00,
    0xc7, 0x49, 0x63, 0x61, 0x72, 0x75, 0x73, 0x00, 0xcb, 0x30, 0x38,
    0x31, 0x39, 0x30, 0x30, 0x30, 0x30, 0x34, 0x33, 0x00, 0xcb, 0x47,
    0x4b, 0x30, 0x38, 0x43, 0x33, 0x41, 0x30, 0x36, 0x36, 0x00, 0x00,
    0xc1, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xc8,
};

// This is a known problematic FRU from a Leaflock where the board area is not
// terminated by a 0xc1 field.
static uint8_t no_area_termination_frudata[] = {
    0x01, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0xfe, 0x01, 0x07, 0x19,
    0x31, 0x7e, 0x86, 0xc7, 0x51, 0x75, 0x61, 0x6e, 0x74, 0x61, 0x00,
...
TEST(FruTest, MultiRecordAreaUpdate) {
  MultiRecordArea record_area;
  MultiRecord record;
  const uint32 kRecordCount = 8;
  for (uint8 i = 0; i < kRecordCount; ++i) {
    record.set_type(i);
  ...
  for (uint8 i = 0; i < record_area.GetMultiRecordCount(); ++i) {
    EXPECT_TRUE(record_area.GetMultiRecord(i, &record));
...
TEST(FruTest, FruFillImageTooSmall) {
  ...
  EXPECT_THAT(result, ::testing::status::StatusIs(
                          ::util::error::INTERNAL,
                          ::testing::HasSubstr("FRU image is too small")));
}
...
TEST(FruTest, FruFillFromGoldenImage) {
  ...
  EXPECT_EQ(std::string("Quanta\0", 7), bia->manufacturer().GetDataAsString());
  EXPECT_EQ(std::string("Icarus\0", 7), bia->product_name().GetDataAsString());
...
TEST(FruTest, FruGetImageGolden) {
  ...
  bia->set_manufacturer(FruFieldFromString(std::string("Quanta\0", 7)));
  bia->set_product_name(FruFieldFromString(std::string("Icarus\0", 7)));
...
TEST(FruTest, FillAndReadFruCustomField) {
  ...
  EXPECT_THAT(DecodeCustomField(cia->custom_fields()[0], &fru_custom_field),
              testing::Not(testing::status::IsOk()));
  EXPECT_THAT(DecodeCustomField(cia->custom_fields()[1], &fru_custom_field),
              testing::Not(testing::status::IsOk()));

  std::string value;
  EXPECT_THAT(cia->GetCustomField(CustomFieldType::kFabId, &value),
              testing::Not(testing::status::IsOk()));
  EXPECT_THAT(cia->GetCustomField(CustomFieldType::kFirmwareId, &value),
              testing::Not(testing::status::IsOk()));

  ASSERT_OK(DecodeCustomField(bia->custom_fields()[0], &fru_custom_field));

  EXPECT_EQ(fru_custom_field.type, CustomFieldType::kUnknownCustomFieldType);
  EXPECT_EQ(fru_custom_field.value, "AB");
  EXPECT_THAT(bia->GetCustomField(CustomFieldType::kFabId, &value),
              testing::Not(testing::status::IsOk()));
  EXPECT_THAT(bia->GetCustomField(CustomFieldType::kFirmwareId, &value),
              testing::Not(testing::status::IsOk()));

  ASSERT_OK(DecodeCustomField(pia->custom_fields()[0], &fru_custom_field));
  EXPECT_EQ(fru_custom_field.type, CustomFieldType::kFabId);
  EXPECT_EQ(fru_custom_field.value, "ABCD");

  ASSERT_OK(DecodeCustomField(pia->custom_fields()[1], &fru_custom_field));
  EXPECT_EQ(fru_custom_field.type, CustomFieldType::kFirmwareId);
  EXPECT_EQ(fru_custom_field.value, "PL00");

  ASSERT_OK(pia->GetCustomField(CustomFieldType::kFabId, &value));
  EXPECT_EQ(value, "ABCD");
  EXPECT_OK(pia->GetCustomField(CustomFieldType::kFirmwareId, &value));
