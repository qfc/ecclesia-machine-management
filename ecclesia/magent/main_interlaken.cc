...
// Redfish server for the Ecclesia Management Agent on Interlaken
#include <cstdlib>
#include <functional>
#include <iostream>
#include <memory>
...
#include "ecclesia/lib/smbios/interlaken/platform_translator.h"
...
#include "ecclesia/magent/redfish/interlaken/redfish_service.h"
...
// For now we leave these comments be until we have updates for interlaken.
// Interlaken machine specific settings.
...
// Info to Build DIMM thermal sensors using hardcoded information for
// SkylakeImc. Each entry is associated with a DIMM. Each sensor device can have
// up to 2 DIMMs. The indices should coincide with the indices of DIMMs. The
// order of the devices (that are mapped to DIMM0, DIMM1, ..., DIMM23),
// according to the motherboard schematic mapping memory channels to silkscreen
// labels, is:
//
...
int main(int argc, char **argv) {
  ...
  ecclesia::SysmodelParams params = {
      .field_translator =
          absl::make_unique<ecclesia::InterlakenSmbiosFieldTranslator>(),
  ...
  ecclesia::InterlakenRedfishService redfish_service(
